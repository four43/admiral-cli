...
function CommandGroup(options) {
	...
	this.commands.map(function (command) {
		if (!(command instanceof Command)) {
			throw new ConfigError("Command of group " + this.name + " was configured wrong, must be Array of Command object, did you use 'new'?");
		}
	}.bind(this));
}
...
CommandGroup.parse = function (args, params, commandGroups, parseParams) {
	...
	for (var i = 0; i < commandGroups.length; i++) {
		...
		commandGroup.commands.forEach(function (command) {
			if(!command) {
				throw new ConfigError("One of the Commands in the command group: " + commandGroup.name + " was invalid, did you use 'new'?");
			}
