	...
	switch (this.type.toLowerCase()) {
		...
		case 'int':
			...
			if (numberRegex.test(value)) {
				...
				throw new InvalidInputError("Option with " + value + " wasn't of type " + option.type);
			}
			break;
		case 'number':
		case 'float':
				...
				case -1:
					...
					while (true) {
						...
						if (args[index] && args[index][0] != '-') {
							...
							var parsedValue = option.validateType(value.toString());
					...
					break;
				case 0:
					throw new ConfigError("A length of 0 doesn't make sense for an option, use a flag instead. Option " + this.name);
					break;
				case 1:
					//Just one
					debug("\t-Single mode");
